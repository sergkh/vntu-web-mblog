SQL (англ. Structured query language — мова структурованих запитів) — декларативна мова для взаємодії користувача з базами даних, що застосовується для формування запитів, оновлення і керування реляційними БД, створення схеми бази даних і її модифікації, системи контролю за доступом до бази даних. 
...
SQL — це діалогова мова для здійснення запиту і внесення змін до бази даних, а також управління базами даних, тому виділяють відповідні частини SQL:
...
Data Definition Language (DDL - Мова опису даних) — це сімейство комп'ютерних мов, що використовуються в СУБД для опису структури даних. Більшість DDL запитів починаються з дієслів:
...
Даний запит створить табличку `students` з вказаними полями. Розглянемо детально поля таблиці:

`id`
: має тип `bigserial`, що є аналогічним до типу `bigint`, який в більшості СУБД описується 8 байтами та приймає значення від	-9223372036854775808 to 9223372036854775807, однак  `bigserial` додатково вказує СУБД інкрементувати значення `id` при кожній вставці до даної таблиці. Якщо значення `bigserial` занадто велике (що в данному випадку досить справдиво оскільки навряд чи система колись зберігатиме мільярди студентів), може стати у нагоді звичайний тип `serial`, розмірність якого відповідає типу `integer`.

    `PRIMARY KEY` вказує, що дане значення буде первичним ключем в даній таблиці. 
    
`email`
: при цьому поле email - має бути унікальним
...
При великих розмірах таблички (зазвичай порядок тисяч, але залежить від конфігурації ПК) запити такого плану будуть виконуватись достатньо довго, а при дуже великих розмірах (зазвичай мільйонів) такі запити фактично взагалі "покладуть" СУБД. Тому що при кожному такому запиті СУБД буде фактично робити повне сканування таблиці (table scan) щоб знайти вказані записи (сервер нать не знає що таке прізвище в табличці тільки одне). Для того щоб прискорити пошук необхідно створити індекс по даному полю, наступним чином:
...
Однак чому ж тобі база даних не створює індекси автоматично для всіх полів? По-перше індекси займають додаткове місце на диску та в пам’яті, якщо індексів багато вони можуть займати більший простір навіть ніж власне самі дані. По-друге три інших DML операції окрім `SELECT` будуть виконуватись довше - тому що постійно потрібно буде крім даних оновлювати ще й індекс, а при унікальному індексу перед вставкою/оновленням перевіряти унікальність по індексу. Ну й останнє, якщо табличка достатньо мала (зазвичай порядо до тисячі) або залежно від специфіки запиту планувальник запитів СУБД може ігнорувати індекси, тому що table scan виконуватиме запит швидше й ефекту від них не буде.
